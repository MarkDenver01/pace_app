...
export default function StatementQuestionPage() {
  ...
  const [editingQuestion, setEditingQuestion] = useState<QuestionTableEntry | null>(null);
      ...
      getActiveCoursesByUniversity(Number(selectedUniversity))
        .then((allCourses) => {
          const filtered = allCourses.filter(
            (c) => String(c.universityId) === selectedUniversity
          );
          setCourses(filtered);
          setSelectedCourse("");
  ...
  // Set course description when course changes
  useEffect(() => {
    if (selectedCourse) {
...
async function handleDelete(id: number) {
  ...
  async function handleSave() {
    ...
    if (!statement.trim()) {
             Swal.fire({
                  icon: "warning",
                  title: "Statement / QuestionRequired",
    ...
    <div className="p-4 flex flex-col gap-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className={cardClass}>
          ...
          <div>
            <Label htmlFor="university" style={valueStyle}>
              University
            </Label>
            <Select
              id="university"
          ...
          <div>
            <Label htmlFor="course" style={valueStyle}>
              Course
            </Label>
            <Select
              id="course"
          ...
          <div>
            <Label htmlFor="courseDesc" style={valueStyle}>
              Course Description
            </Label>
            <Textarea
              id="courseDesc"
              ...
              {paginatedData.length > 0 ? (
                paginatedData.map((entry) => (
                  ...
                  >
                    ...
                    <td className="p-3">{entry.university}</td>
      ...
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mt-6 text-sm text-gray-600">
          <span>
            ...
            of <span className="font-semibold text-gray-800">{tableData.length}</span>{" "}
            entries
          </span>
          <Pagination
      ...
      <Modal show={isEditModalOpen} onClose={closeEditModal} popup>
        ...
        className="p-6"
        style={{
          minHeight: "550px",  // increase this as needed
          maxHeight: "80vh",
          overflowY: "auto",
        }}
        >
          {editingQuestion && (
            <EditQuestionForm
              ...
              initialUniversityId={
                universities.find(
                  (u) => u.universityName === editingQuestion.university
                )?.universityId ?? null
              }
              initialCourseId={
