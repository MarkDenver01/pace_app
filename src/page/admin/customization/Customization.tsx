...
export default function Customization() {
    ...
    useEffect(() => {
        const fetchCustomization = async () => {
            try {
                // Fetch the theme with a default ID (e.g., 1)
                const data: CustomizationResponse = await getTheme(1);
                console.log("[Customization] Loaded theme from API:", data.themeName);

                // Use type guard to safely check themeName validity
                const safeTheme = isTheme(data.themeName) ? data.themeName : "light";
                ...
                // Handle the case where the theme doesn't exist yet
    ...
    const handleSave = async () => {
        try {
            const request: CustomizationRequest = {
                id: themeId, // Include the ID for updates
                themeName,
                aboutText,
            ...
            setLogoFile(null); // Clear the file input state after successful save
        } catch (err) {
